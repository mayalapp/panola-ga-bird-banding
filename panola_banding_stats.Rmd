...
# input: my_spec - a list of the species we are interested in 
# output: BPNH for each year for each species in my_spec 
calc_bpnh <- function(my_spec){
  out <- all_data %>%  
...
calc_bpnh(most_observed_birds) %>% plot_bpnh() 
```
...
```{r} 
calc_bpnh(key_grasslands) %>% plot_bpnh() 
...
```{r}
calc_bpnh(my_species) %>% plot_bpnh() 
```
